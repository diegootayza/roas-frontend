...
<script lang="ts">
    ...
    import { createLesson, deleteLesson, updateLesson } from '@/graphql/Lesson'
    ...
    export default defineComponent({
        ...
        setup(props, { emit }) {
            ...
            const { mutate: createMutate } = useMutation(createLesson)
            const { mutate: updateMutate } = useMutation(updateLesson)
            const { mutate: deleteMutate } = useMutation(deleteLesson)
            ...
            const submitForm = async () => {
                ...
                if (method.value === 'CREATE') {
                    const createResponse = await createMutate({ input: { ...submitData, module } })

                    if (createResponse) {
                        const createLesson = createResponse.data.createLesson
                ...
                if (method.value === 'UPDATE') {
                    const updateResponse = await updateMutate({ id: id.value, input: { ...submitData, module } })

                    if (updateResponse) {
                        const updateLesson = updateResponse.data.updateLesson
                ...
                if (method.value === 'DELETE') {
                    ...
                    if (deleteResponse) {
                        const deleteLesson = deleteResponse.data.deleteLesson
